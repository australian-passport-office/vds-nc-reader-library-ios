//
//  VDSAuthenticatorTests.swift
//  VDSNCChecker
//
//  Copyright (c) 2021, Commonwealth of Australia. vds.support@dfat.gov.au
//
//  Licensed under the Apache License, Version 2.0 (the "License"); you may not
//  use this file except in compliance with the License. You may obtain a copy
//  of the License at:
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
//  Unless required by applicable law or agreed to in writing, software
//  distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
//  WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the
//  License for the specific language governing permissions and limitations
//  under the License.

import XCTest
@testable import VDSNCChecker

class VDSAuthenticatorTests: XCTestCase {
    
    // SHA256 hash of our CSCA certificate data
    let cscaCertSHA256Hash = "35623a89f6b40cb4888e05147f7d16d27f7bf754aad563c37fef6e1a0e7ff5cf"
    
    // CSCA certificate
    let cscaCertData = Data(base64Encoded: "")!
    
    // BSC certificate
     let bscCertData = Data(base64Encoded: "")!
    
    // CRL
    let crlData = Data(base64Encoded: "")!
    
    // Valid VDS JSON
    let validVDSJson = """
{
  "data": {
    "hdr": {
      "is": "AUS",
      "t": "icao.vacc",
      "v": 1
    },
    "msg": {
      "pid": {
        "dob": "1977-05-16",
        "i": "PF0911009",
        "n": "CITIZEN  JANE CATHERINE",
        "sex": "F"
      },
      "uvci": "VB0009990038",
      "ve": [
        {
          "des": "XM68M6",
          "dis": "RA01.0",
          "nam": "Pfizer Comirnaty",
          "vd": [
            {
              "adm": "Community Health",
              "ctr": "AUS",
              "dvc": "2021-06-13",
              "lot": "EP2163",
              "seq": 1
            },
            {
              "adm": "General Practitioner",
              "ctr": "AUS",
              "dvc": "2021-07-01",
              "lot": "ER7449",
              "seq": 2
            }
          ]
        }
      ]
    }
  },
  "sig": {
    "alg": "ES256",
    "cer": "-mPqubbW3mjs0DTeRKrfVTSkkZNgOGj_DB_fo3p8qGy8UVgT4DQRVhIo1IwUDAWBgdngQgBAQYCBAswCQIBADEEEwJOVjAVBgNVHSUBAf8ECzAJBgdngQgBAQ4CMB8GA1UdIwQYMBaAFDYXwef1Z5VxLjd1cI5VgzGG6TgOMA0GCSqGSIb3DQEBCwUAA4ICAQCh_Qc5i6-vewGqinR9EdUpsl0P4jqg0pdx7hyOtPgYOwbTOegJyZOjyWZyuLlxGYuvCHqbrnATMedoIoUJzt8GxHA-4v5TUN2yEbRFXev8ur_0Y3uF4WXFr93Zl0LV78PBNZwXKfZEC6oTN_eVgtR37GdnYsWno0SuhR4fJo8JC_blivas8BJt78Hg8VhvWSK3uT0T58eYQjQhbsXV-BxJ2kSspdvkUF6-arLHh6DVS3ATPAGIm6fEvF4AxnLq5OSHOC3zZR0SR9XntYxEwjo_bW8O0Se8qa5mIBpXmvlwh0Ij6sqVwEskvkM30GmQGfZh5VjFujN2AZnwpjOjK0R-JvR3u6jsBJqVMgm75HgezOzayNiaqzhitrgg5KpO3gK_j3C-Doj5iPAm7I_63GyjUi8ZnqVUZ37UxM19uX2SvhTTQ70nL-zHNfHOyBXJgzMi4Zkor2uagHPz-W1XvNVwGEfFAu-nEyIOKBndHwnvSomL54yBv83X2yAQsoYggU18LNXMHUonTJ_ug7FU0LEX3qA1TeARJ4WBFNjysrBXQepVLowcbtvrhLFjocHjmCp3z17xUoKGI6daajCbvedXgeeSWSD5CuMAXpdN3Yml7VdW7PCK4DD0E_raw6d_wKNGSYAh0TBpNLxnunquai-gFIjgf4iRoys5F35KwmvpZw==",
    "sigvl": "FM5nk4TntHUSrVewwK3mqhmyXxAlJgKbu2qt9ZZI6U56PKVkzfDmIQKhURAlvnCKv9SfzmgtLCUxQFpbf2EuXQ=="
  }
}
"""
    
    
    // Invalid VDS JSON - identical to the valid JSON, except the name was modified (JANE to JANEX) to make the signature invalid
    let invalidVDSJson = """
{
  "data": {
    "hdr": {
      "is": "AUS",
      "t": "icao.vacc",
      "v": 1
    },
    "msg": {
      "pid": {
        "dob": "1977-05-16",
        "i": "PF0911009",
        "n": "CITIZEN  JANEX CATHERINE",
        "sex": "F"
      },
      "uvci": "VB0009990038",
      "ve": [
        {
          "des": "XM68M6",
          "dis": "RA01.0",
          "nam": "Pfizer Comirnaty",
          "vd": [
            {
              "adm": "Community Health",
              "ctr": "AUS",
              "dvc": "2021-06-13",
              "lot": "EP2163",
              "seq": 1
            },
            {
              "adm": "General Practitioner",
              "ctr": "AUS",
              "dvc": "2021-07-01",
              "lot": "ER7449",
              "seq": 2
            }
          ]
        }
      ]
    }
  },
  "sig": {
    "alg": "ES256",
    "cer": "-mPqubbW3mjs0DTeRKrfVTSkkZNgOGj_DB_fo3p8qGy8UVgT4DQRVhIo1IwUDAWBgdngQgBAQYCBAswCQIBADEEEwJOVjAVBgNVHSUBAf8ECzAJBgdngQgBAQ4CMB8GA1UdIwQYMBaAFDYXwef1Z5VxLjd1cI5VgzGG6TgOMA0GCSqGSIb3DQEBCwUAA4ICAQCh_Qc5i6-vewGqinR9EdUpsl0P4jqg0pdx7hyOtPgYOwbTOegJyZOjyWZyuLlxGYuvCHqbrnATMedoIoUJzt8GxHA-4v5TUN2yEbRFXev8ur_0Y3uF4WXFr93Zl0LV78PBNZwXKfZEC6oTN_eVgtR37GdnYsWno0SuhR4fJo8JC_blivas8BJt78Hg8VhvWSK3uT0T58eYQjQhbsXV-BxJ2kSspdvkUF6-arLHh6DVS3ATPAGIm6fEvF4AxnLq5OSHOC3zZR0SR9XntYxEwjo_bW8O0Se8qa5mIBpXmvlwh0Ij6sqVwEskvkM30GmQGfZh5VjFujN2AZnwpjOjK0R-JvR3u6jsBJqVMgm75HgezOzayNiaqzhitrgg5KpO3gK_j3C-Doj5iPAm7I_63GyjUi8ZnqVUZ37UxM19uX2SvhTTQ70nL-zHNfHOyBXJgzMi4Zkor2uagHPz-W1XvNVwGEfFAu-nEyIOKBndHwnvSomL54yBv83X2yAQsoYggU18LNXMHUonTJ_ug7FU0LEX3qA1TeARJ4WBFNjysrBXQepVLowcbtvrhLFjocHjmCp3z17xUoKGI6daajCbvedXgeeSWSD5CuMAXpdN3Yml7VdW7PCK4DD0E_raw6d_wKNGSYAh0TBpNLxnunquai-gFIjgf4iRoys5F35KwmvpZw==",
    "sigvl": "FM5nk4TntHUSrVewwK3mqhmyXxAlJgKbu2qt9ZZI6U56PKVkzfDmIQKhURAlvnCKv9SfzmgtLCUxQFpbf2EuXQ=="
  }
}
"""
    
    // Corrupt/non-VDS JSON
    let nonVDSJson = """
{
    "isThisAVDS": false
}
"""
    
    
    func testValidVDS() throws {
        // Decode VDS
        let vds = try? VDSReader().decodeVDSFrom(jsonString: validVDSJson)
        XCTAssertNotNil(vds)
        
        // Verify VDS
        let authenticator = VDSAuthenticator()
        let crl = CRL(crlData: crlData)
        let cscaCertificate = CSCACertificate(data: cscaCertData, integrityHash: cscaCertSHA256Hash, crl: crl)
        let isVdsValid = try? authenticator.verify(
            vds: vds!,
            withCscaCertificate: cscaCertificate)
        
        XCTAssertNotNil(isVdsValid)
        XCTAssertEqual(isVdsValid, true)
    }
    
    func testInvalidVDS() throws {
        // Decode VDS
        let vdsReader = VDSReader()
        let vds = try? vdsReader.decodeVDSFrom(jsonString: invalidVDSJson)
        XCTAssertNotNil(vds)
        
        // Verify VDS
        let authenticator = VDSAuthenticator()
        let crl = CRL(crlData: crlData)
        let cscaCertificate = CSCACertificate(data: cscaCertData, integrityHash: cscaCertSHA256Hash, crl: crl)
        
        XCTAssertThrowsError(try authenticator.verify(
            vds: vds!,
            withCscaCertificate: cscaCertificate)) { error in
                XCTAssertEqual(error as? VDSVerifyError, VDSVerifyError.verifyVDSSignatureFailed)
            }
    }
    
    func testNonVDS() throws {
        // Decode VDS
        let vdsReader = VDSReader()
        XCTAssertThrowsError(try vdsReader.decodeVDSFrom(jsonString: nonVDSJson)) { error in
            XCTAssertEqual(error as? VDSDecodeError, VDSDecodeError.jsonDecoding)
        }
    }
    
    func testEmptyCRL() throws {
        // Note: This test is expected to fail due to the CRL not having any data
        // Decode VDS
        let vdsReader = VDSReader()
        let vds = try? vdsReader.decodeVDSFrom(jsonString: invalidVDSJson)
        XCTAssertNotNil(vds)
        
        // Verify VDS
        let authenticator = VDSAuthenticator()
        let crl = CRL(crlData: Data())
        let cscaCertificate = CSCACertificate(data: cscaCertData, integrityHash: cscaCertSHA256Hash, crl: crl)
        
        XCTAssertThrowsError(try authenticator.verify(
            vds: vds!,
            withCscaCertificate: cscaCertificate)) { error in
                XCTAssertEqual(error as? VDSVerifyError, VDSVerifyError.loadCRLFailed)
            }
    }
    
    func testVDSCanonicalization() throws {
        
        let inputStr = validVDSJson
        let expectedStr = """
        {"data":{"hdr":{"is":"AUS","t":"icao.vacc","v":1},"msg":{"pid":{"dob":"1977-05-16","i":"PF0911009","n":"CITIZEN  JANE CATHERINE","sex":"F"},"uvci":"VB0009990038","ve":[{"des":"XM68M6","dis":"RA01.0","nam":"Pfizer Comirnaty","vd":[{"adm":"Community Health","ctr":"AUS","dvc":"2021-06-13","lot":"EP2163","seq":1},{"adm":"General Practitioner","ctr":"AUS","dvc":"2021-07-01","lot":"ER7449","seq":2}]}]}},"sig":{"alg":"ES256","cer":"-mPqubbW3mjs0DTeRKrfVTSkkZNgOGj_DB_fo3p8qGy8UVgT4DQRVhIo1IwUDAWBgdngQgBAQYCBAswCQIBADEEEwJOVjAVBgNVHSUBAf8ECzAJBgdngQgBAQ4CMB8GA1UdIwQYMBaAFDYXwef1Z5VxLjd1cI5VgzGG6TgOMA0GCSqGSIb3DQEBCwUAA4ICAQCh_Qc5i6-vewGqinR9EdUpsl0P4jqg0pdx7hyOtPgYOwbTOegJyZOjyWZyuLlxGYuvCHqbrnATMedoIoUJzt8GxHA-4v5TUN2yEbRFXev8ur_0Y3uF4WXFr93Zl0LV78PBNZwXKfZEC6oTN_eVgtR37GdnYsWno0SuhR4fJo8JC_blivas8BJt78Hg8VhvWSK3uT0T58eYQjQhbsXV-BxJ2kSspdvkUF6-arLHh6DVS3ATPAGIm6fEvF4AxnLq5OSHOC3zZR0SR9XntYxEwjo_bW8O0Se8qa5mIBpXmvlwh0Ij6sqVwEskvkM30GmQGfZh5VjFujN2AZnwpjOjK0R-JvR3u6jsBJqVMgm75HgezOzayNiaqzhitrgg5KpO3gK_j3C-Doj5iPAm7I_63GyjUi8ZnqVUZ37UxM19uX2SvhTTQ70nL-zHNfHOyBXJgzMi4Zkor2uagHPz-W1XvNVwGEfFAu-nEyIOKBndHwnvSomL54yBv83X2yAQsoYggU18LNXMHUonTJ_ug7FU0LEX3qA1TeARJ4WBFNjysrBXQepVLowcbtvrhLFjocHjmCp3z17xUoKGI6daajCbvedXgeeSWSD5CuMAXpdN3Yml7VdW7PCK4DD0E_raw6d_wKNGSYAh0TBpNLxnunquai-gFIjgf4iRoys5F35KwmvpZw==","sigvl":"FM5nk4TntHUSrVewwK3mqhmyXxAlJgKbu2qt9ZZI6U56PKVkzfDmIQKhURAlvnCKv9SfzmgtLCUxQFpbf2EuXQ=="}}
        """
        let data = try JsonCanonicalizer.canonicalize(json: inputStr)
        XCTAssertNotNil(data)
        let outputStr = String(data: data, encoding: String.Encoding.utf8)
        XCTAssertNotNil(outputStr)
        XCTAssertEqual(outputStr!, expectedStr)
    }
    
    func testCscaHash() throws {
        
        let authenticator = VDSAuthenticator()
        
        XCTAssertNoThrow(try authenticator.verifyCSCACertHash(cscaCertData: cscaCertData, cscaCertSHA256Hash: cscaCertSHA256Hash))
        
    }
    
    func testAKISKIMatch() throws {

        let authenticator = VDSAuthenticator()

        XCTAssertNoThrow(try authenticator.verifyAKIMatchInBSCCert(bscCertData, withSKIInCsca: cscaCertData))
    }
    
    func testCertPath() throws {

        let authenticator = VDSAuthenticator()

        XCTAssertNoThrow(try authenticator.verifyPath(inBscCert: bscCertData, forCscaCert: cscaCertData))
    }
}
